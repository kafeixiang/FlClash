...
/// [PageTraversalPolicy] group hosting [PageFocusScope] page content, so the
/// FAB is first in the tab order, tabbing past the page reaches the
...
void main() {
  ...
  testWidgets('FAB shows a visible focus ring while focused', (tester) async {
    await pumpWithOutsideFocus(tester, _buildPage());
    ...
    expect(ringSide().width, 0, reason: 'no ring before focus');
    ...
    expect(
      ringSide().width,
      2.5,
      reason: 'a visible ring should mark the focused FAB',
    );

    await tester.sendKeyEvent(LogicalKeyboardKey.tab);
    await tester.pumpAndSettle();
    expect(_focusedItemKey(), 'item0');
    expect(ringSide().width, 0, reason: 'ring disappears after focus moves');
  });
}
