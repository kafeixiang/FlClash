...
- `/ping` is loopback-only and requires no request token. The Helper compares the requested SHA256 with its embedded value
  ...
  If `/start` then reports `coreVerificationFailed` (the on-disk Core no longer matches the SHA the Helper and manifest
  agree on), the launcher degrades to the direct Core rather than failing the launch. The Helper spawns nothing before
  verification, so this retry leaves no Helper-managed Core behind.
