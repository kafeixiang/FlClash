...
extension CodeForgeControllerEditing on CodeForgeController {
  ...
  void replaceRange(int start, int end, String replacement) {
    ...
    _recordLineEdit(dirtyLine!, deletedText, replacement);
  ...
  void unindent() {
    if (selection.baseOffset != selection.extentOffset) {
      ...
      final firstRemoved = lines.first.startsWith(tabSpace)
          ? tabSize
          : (RegExp(r'^ +').stringMatch(lines.first)?.length ?? 0);
      final scalarLineStart = text.toScalarOffset(lineStart);
      final newSelection = _withEnds(
        selection,
        max(selection.start - firstRemoved, scalarLineStart),
        max(selection.end - removedChars, scalarLineStart),
      );
