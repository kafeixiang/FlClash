...
class _ClashProvidersViewState extends ConsumerState<ClashProvidersView> {
  ...
  void _put(ClashProvider provider, {ClashProvider? previous}) {
    _providersAction.putProvider(provider, previous: previous);
  }
  ...
  Future<void> _handleImportFromFile() async {
    ...
    await provider.saveContent(bytes);
    _put(provider);
  }
  ...
  Future<void> _handleEditorSave(
    ...
    await next.saveContent(utf8.encode(content));
    _put(next, previous: provider);
    if (mounted) {
